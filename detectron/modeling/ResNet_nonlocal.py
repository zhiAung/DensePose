...
import detectron.modeling.nonlocal_helper as nonlocal_helper
...
def add_ResNet_convX_body(model, block_counts, freeze_at=2):
    ...
    s, dim_in = add_stage_nonlocal(
        model, 'res3', s, n2, dim_in, 512, dim_bottleneck * 2, 1, nonlocal_mod=2, nonlocal_name='nonlocal_conv3'
    )
    if freeze_at == 3:
        model.StopGradient(s, s)
    s, dim_in = add_stage_nonlocal(
        model, 'res4', s, n3, dim_in, 1024, dim_bottleneck * 4, 1, nonlocal_mod=2, nonlocal_name='nonlocal_conv4'
    )
    if freeze_at == 4:
        model.StopGradient(s, s)
